...
// C++ port of the swfec sliding-window FEC. Reference implementation:
// crates/swfec in the fpv-transport repo. The wire format and the
// coefficient PRNG are FROZEN PROTOCOL — changing them breaks interop with
// the Rust implementation and any deployed node. Differential tests in
// fec_swfec_test.cpp pin byte-exact equality against committed vectors.
...
// Wire constants (must match crates/swfec/src/wire.rs)
enum {
...
// Must generate byte-identical output to crates/swfec/src/coeff.rs.
class CoeffGen {
public:
...
// Systematic sliding-window encoder. Mirrors crates/swfec/src/encoder.rs:
...
// Incremental Gaussian-elimination decoder. Mirrors crates/swfec/src/decoder.rs:
