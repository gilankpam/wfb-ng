...
// C++ port of the swfec sliding-window FEC (from a Rust reference
// implementation). The wire format and the coefficient PRNG are FROZEN
// PROTOCOL — changing them breaks interop with any deployed node.
// Differential tests in fec_swfec_test.cpp pin byte-exact equality against
// vectors generated by the reference implementation (run when a local
// test_vectors/ directory is present).
